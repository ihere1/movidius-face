...
def detect_face(img, minsize, threshold = [0.405, 0.8473, 0.8473], factor = 0.709, scale_factor = 2):
    # im: input image
    ...
    for j in range(len(scales)):
        ...
        out = sess.run((pnet_output0, pnet_output1), feed_dict={pnet_input: img_x})
        boxes, _ = generateBoundingBox(out[0][0, :, :, 1] - out[0][0, :, :, 0], out[1][0, :, :, :].copy(), scale, threshold[0])
    ...
    if numbox > 0:
        ...
        out = sess.run((rnet_output0, rnet_output1), feed_dict={rnet_input: tempimg1})
    ...
    if numbox > 0:
        ...
        tempimg = (tempimg - 127.5) * 0.0078125
        tempimg1 = np.transpose(tempimg, (3, 1, 0, 2))
        out = sess.run((onet_output0, onet_output1), feed_dict={onet_input: tempimg1})
        out0 = np.transpose(out[0])
        out1 = np.transpose(out[1])
        score = out0[1, :] - out0[0, :]
